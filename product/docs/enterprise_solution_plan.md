...
> **Document status — future product vision.** This is the post-hackathon target architecture. It must not be read as a description of the protected evaluator implementation, which is intentionally a compact offline direct-ridge inference pipeline. See [`current_implementation.md`](current_implementation.md).
