...
Meta's `conversion` field is treated only as the supplied platform-attributed revenue proxy. Explicit campaign-name patterns map prospecting, remarketing, and DPA campaigns into operational Meta types; a reviewed `campaign_taxonomy.csv` can override that mapping. Neither assumption establishes cross-platform attribution or causality.
